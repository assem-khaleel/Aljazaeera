...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			if ($this->user->hasPermission('access', 'catalog/branch')) {
				$catalog[] = array(
					'name'	   => $this->language->get('text_branch'),
					'href'     => $this->url->link('catalog/branch', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}

			if ($this->user->hasPermission('access', 'catalog/page_group')) {
				$catalog[] = array(
					'name'	   => $this->language->get('text_page_group'),
					'href'     => $this->url->link('catalog/page_group', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
